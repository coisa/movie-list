 ...
 */
final class SearchRequestHandlerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->requestHandler = new SearchRequestHandler(
            $this->repository->reveal(),
            $this->templateRenderer->reveal()
        );
    }

    public function testSearchRequestHandlerImplementsRequestHandlerInterface(): void
    {
        $this->assertInstanceOf(RequestHandlerInterface::class, $this->requestHandler);
    }

    public function testHandleWillReturnHtmlResponseWithQueriedMoviesTemplate(): void
    {
        $query   = \uniqid('query', true);
        $movies  = [\uniqid('movies', true)];
        $content = \uniqid('content', true);

        $this->serverRequest->getQueryParams()->willReturn(\compact('query'));
        $this->repository->search($query)->willReturn($movies)->shouldBeCalledOnce();
        $this->templateRenderer->render(
            'pages::search',
            \compact('movies', 'query')
